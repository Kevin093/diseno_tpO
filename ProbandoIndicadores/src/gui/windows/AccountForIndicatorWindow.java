...
public class AccountForIndicatorWindow extends SimpleWindow<AccountVM> {
	public WindowOwner parentWO;

	public AccountForIndicatorWindow(WindowOwner parent) {
	...
	public void createContents(Panel Master) {
		...
		new Button(dataForm).setCaption("Volver").onClick(() -> this.closeWindow());
	}
	...
	public void closeWindow() {
		...
		((IndicatorWindow) parentWO).getModelObject()
				.setReceivedAccount(this.getModelObject().getChosenEnterpriseAccount());
		((IndicatorWindow) parentWO).getModelObject()
				.setReceivedAccountPeriod(this.getModelObject().getChosenEnterpriseAccountPeriod());
		this.close();
	}
